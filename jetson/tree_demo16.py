...
# Add serial control code after the imports
class Command(Enum):
    FORWARD = auto()
    BACKWARD = auto()
    LEFT = auto()
...
last_command_time = 0  # Track last command time for rate limiting
...
def send_command(command: Command):
    ...
    if command in command_map:
        try:
            ...
            logging.debug(f"📡 Sent command: {command.name}")
...
# Update process_autonomous_movement to use sync movement commands
...
async def handle_prompt_update(request):
    ...
    try:
        ...
        logging.info(f"🎯 Target (Survivor indicators): {', '.join(target_objects)}")
        if obstacles:
...
if __name__ == "__main__":
    ...
    logging.info(f"🎯 Default Target (Survivor indicators): {', '.join(default_targets)}")
